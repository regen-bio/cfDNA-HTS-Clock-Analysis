...
def _plot_main(data_fname: str, plot_fname: str, highlight_clocks: set[str] = None,
	*, metric_cfgs: Sequence[MetricCfg] = None, repl_group_dataset: str = None,
	skip: bool = False,
):
	if skip:
		pylib.logger.info(f"skip: {plot_fname}")
		return

	# config
	if metric_cfgs is None:
		metric_cfgs = [
			MetricCfg("mae", "MAE", unit="years"),
		]
	key_field_cfgs = KeyFieldList([
	...
	# load results
	key_cfgs, clocks, results = _load_results(data_fname)
	clock_cfgs = pylib.ClockCfgLib.from_json(select=highlight_clocks)
	if repl_group_dataset is not None:
		repl_group = pylib.dataset.ReplGroup.load_dataset(repl_group_dataset)
	else:
		repl_group = None
	...
	figure.savefig(plot_fname, dpi=600)
...
if __name__ == "__main__":
	_plot_main(
		data_fname="srrsh_filter_imput_transfer.age_pred.pkl",
		plot_fname="srrsh_filter_imput_transfer.svg",
		skip=True,
	)
	_plot_main(
		data_fname="GSE186458_filter_imput_transfer.age_pred.pkl",
		plot_fname="fig_s16.svg",
	)
	_plot_main(
		data_fname="BUCCAL_TWIST_filter_imput_transfer.age_pred.pkl",
		plot_fname="fig_s17.svg",
		metric_cfgs=[
			MetricCfg("mae", "MAE", unit="years"),
			MetricCfg("repl_mad", "RD", unit="years"),
		],
		repl_group_dataset="BUCCAL_TWIST",
	)
